...
class Auditor(LambdaBase):
    ...
    def __repr__(self):
        return ("awsscripter.audit.audit.Audit("
                ...
                ")".format(project_code=self.project_code, region=self.connection_manager.region,
                           ...
                           awsscripter_user_data=self.awsscripter_user_data, hooks=self.hooks,
                           s3_details=self.s3_details, dependencies=self.dependencies, role_arn=self.role_arn,
                           protected=self.protected, tags=self.tags, notifications=self.notifications,
    ...
    def handle(self, event, context):
        ...
        control2 = []
        control2.append(self.control_2_1_ensure_cloud_trail_all_regions(cloud_trail))
        control2.append(self.control_2_2_ensure_cloudtrail_validation(cloud_trail))
        control2.append(self.control_2_3_ensure_cloudtrail_bucket_not_public(cloud_trail))
        control2.append(self.control_2_4_ensure_cloudtrail_cloudwatch_logs_integration(cloud_trail))
        control2.append(self.control_2_5_ensure_config_all_regions(regions))
        control2.append(self.control_2_6_ensure_cloudtrail_bucket_logging(cloud_trail))
        control2.append(self.control_2_7_ensure_cloudtrail_encryption_kms(cloud_trail))
        control2.append(self.control_2_8_ensure_kms_cmk_rotation(regions))

        control3 = []
        control3.append(self.control_3_1_ensure_log_metric_filter_unauthorized_api_calls(cloud_trail))
        control3.append(self.control_3_2_ensure_log_metric_filter_console_signin_no_mfa(cloud_trail))
        control3.append(self.control_3_3_ensure_log_metric_filter_root_usage(cloud_trail))
        control3.append(self.control_3_4_ensure_log_metric_iam_policy_change(cloud_trail))
        control3.append(self.control_3_5_ensure_log_metric_cloudtrail_configuration_changes(cloud_trail))
        control3.append(self.control_3_6_ensure_log_metric_console_auth_failures(cloud_trail))
        control3.append(self.control_3_7_ensure_log_metric_disabling_scheduled_delete_of_kms_cmk(cloud_trail))
        control3.append(self.control_3_8_ensure_log_metric_s3_bucket_policy_changes(cloud_trail))
        control3.append(self.control_3_9_ensure_log_metric_config_configuration_changes(cloud_trail))
        control3.append(self.control_3_10_ensure_log_metric_security_group_changes(cloud_trail))
        control3.append(self.control_3_11_ensure_log_metric_nacl(cloud_trail))
        control3.append(self.control_3_12_ensure_log_metric_changes_to_network_gateways(cloud_trail))
        control3.append(self.control_3_13_ensure_log_metric_changes_to_route_tables(cloud_trail))
        control3.append(self.control_3_14_ensure_log_metric_changes_to_vpc(cloud_trail))
        control3.append(self.control_3_15_verify_sns_subscribers(cloud_trail))
    ...
    def control_1_14_root_hardware_mfa_enabled(self):
        ...
        if response['SummaryMap']['AccountMFAEnabled'] == 1:
            mfa_kwargs = {"operation_name": 'list_virtual_mfa_devices'}
            paginator = self.connection_manager.call(service='iam', command='get_paginator', kwargs=mfa_kwargs)
    ...
    def control_1_16_no_policies_on_iam_users(self):
        ...
        scored = True
        group_kwargs = {"operation_name": 'list_users'}
        ...
        for n in pagedResult:
            ...
            policy_kwargs = {"UserName": UserName, "MaxItems": 1}
            policies = self.connection_manager.call(service="iam", command="list_user_policies", kwargs=policy_kwargs)
    ...
    def control_1_22_ensure_incident_management_roles(self):
        ...
        offenders = []
        try:
            policy_kwargs = {"PolicyArn": 'arn:aws:iam::aws:policy/AWSSupportAccess'}
            response = self.connection_manager.call(
                service='iam',
    ...
    def control_1_23_no_active_initial_access_keys_with_iam_user(self, credreport):
        ...
        for n, _ in enumerate(credreport):
            if (credreport[n]['access_key_1_active'] or credreport[n]['access_key_2_active'] == 'true') and n > 0:
                user_kwargs = {"UserName": str(credreport[n]['user'])}
                response = self.connection_manager.call(
                    service='iam',
    ...
    def control_2_2_ensure_cloudtrail_validation(self, cloudtrail):
        """Summary
    ...
    def control_2_8_ensure_kms_cmk_rotation(self, regions):
        ...
        for n in regions:
            ...
            # response_iterator = paginator.paginate()
            cloud_kwargs = {"operation_name": 'list_keys',}
            paginator = self.connection_manager.call(
                service='kms',
                command='get_paginator',
                kwargs=cloud_kwargs
    ...
    def setRegion(self, region, iam_role=None):
        self.connection_manager(region, iam_role=None)
            ...
            for o in n:
                try:
                    if o['CloudWatchLogsLogGroupArn']:
                        ...
                        cloud_kwargs = {'LogGroupName': group, }
                        ...
                        for p in filters['metricFilters']:
                            ...
                            if find_in_string(patterns, str(p['filterPattern'])):
                                ...
                                """                    
                                snsClient = self.connection_manager.client('sns', region_name=m)
        ...
        for m, n in cloudtrails.items():
            ...
            for o in n:
                try:
                    if o['CloudWatchLogsLogGroupArn']:
                        group = re.search('log-group:(.+?):', o['CloudWatchLogsLogGroupArn']).group(1)
                        # self.setRegion(regions, iam_role=None)
                        cloud_kwargs = {
                            'logGroupName': group,
                        ...
                        for p in filters['metricFilters']:
                            ...
                            if find_in_string(patterns, str(p['filterPattern'])):
                                ...
                                subscribers = self.connection_manager.call(
                                    service='sns',
                                    command='list_describe_alarms_for_metric',
                                    kwargs=cloud_kwargs
                                )
                                """
    ...
    def control_3_4_ensure_log_metric_iam_policy_change(self, cloudtrails):
            ...
            for o in n:
                try:
                    if o['CloudWatchLogsLogGroupArn']:
                        ...
                        filters = self.connection_manager.call(
                            service='logs',
                            command='describe_metric_fiters',
                            kwargs=cloud_kwargs
                        )
                        # client = boto3.client('logs', region_name=m)
                        """
                        ...
                        for p in filters['metricFilters']:
                            ...
                            if find_in_string(patterns, str(p['filterPattern'])):
                                ...
                                TopicArn = response['MetricAlarms'][0]['AlarmActions'][0]
                                cloud_kwargs = {'TopicArn': 'TopicArn',
        ...
        return {'Result': result, 'failReason': failReason, 'Offenders': offenders, 'ScoredControl': scored,
                'Description': description,
                'ControlId': control}  # 3.5 Ensure a log metric filter and alarm exist for CloudTrail configuration changes (Scored)
                    ...
                    if o['CloudWatchLogsLogGroupArn']:
                        ...
                        for p in filters['metricFilters']:
                            ...
                            if find_in_string(patterns, str(p['filterPattern'])):
                                MetricName = p['metricTransformations'][0]['metricName'],
                                Namespace = p['metricTransformations'][0]['metricNamespace']
                                cloud_kwargs = {'MetricName': MetricName, 'Namespace': Namespace}
                                response = self.connection_manager.call(
                                    service='cloudwatch',
                                    command='describe_alarms_for_metric',
                                    kwargs=cloud_kwargs
                    ...
                    if o['CloudWatchLogsLogGroupArn']:
                        ...
                        for p in filters['metricFilters']:
                            ...
                            if find_in_string(patterns, str(p['filterPattern'])):
                                ...
                                Namespace = p['metricTransformations'][0]['metricNamespace']
                                cloud_kwargs = {'MetricName': MetricName, 'Namespace': Namespace}
                                response = self.connection_manager.call(service='cloudwatch',
                                    command='describe_alarms_for_metric', kwargs=cloud_kwargs)
                                """
                                cwclient = connection_manager.client('cloudwatch', region_name=m)
                                response = cwclient.describe_alarms_for_metric(
                                MetricName=p['metricTransformations'][0]['metricName'],
    ...
    def control_3_15_verify_sns_subscribers(self, cloudtrails):
        """Summary
    ...
    def control_4_3_ensure_flow_logs_enabled_on_all_vpc(self, regions):
        ...
        for n in regions:
            ...
            vpcs = self.connection_manager.call(service="ec2", command="describe_vpcs", kwargs=ec2_kwargs)
            for m in vpcs['Vpcs']:
    ...
    def send_results_to_sns(url):
        ...
        client.publish(TopicArn=Auditor.SNS_TOPIC_ARN, Subject="AWS CIS Benchmark report - " + str(time.strftime("%c")),
