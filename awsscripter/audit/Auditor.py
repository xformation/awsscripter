...
from awsscripter.audit.region_n import region_n
...
class Auditor(LambdaBase):
    ...
    def handle(self, event, context):
        ...
        cld_trails = CloudTrail()
        regionlist = cld_trails.get_regions()
        cloudtrails = cld_trails.get_cloudtrails(regionlist)
        rg = region_n()
        ...
        #control4 = []
        #control4.append(self.control_4_1_ensure_ssh_not_open_to_world(regionlist))
        #control4.append(self.control_4_2_ensure_rdp_not_open_to_world(regionlist))
        #control4.append(self.control_4_3_ensure_flow_logs_enabled_on_all_vpc(regionlist))
        #control4.append(self.control_4_4_ensure_default_security_groups_restricts_traffic(regionlist))
        #control4.append(self.control_4_5_ensure_route_tables_are_least_access(regionlist))
        # Join results
        controls = []
    ...
    def control_1_23_no_active_initial_access_keys_with_iam_user(self, credreport):
        ...
        for n, _ in enumerate(credreport):
            ...
            if (credreport[n]['access_key_1_active'] or credreport[n]['access_key_2_active'] == 'true') and n > 0:
                ...
                response.UserName = str(credreport[n]['user'])
                    ...
                    if o['CloudWatchLogsLogGroupArn']:
                        ...
                        for p in filters['metricFilters']:
                            ...
                            if find_in_string(patterns, str(p['filterPattern'])):
                                ...
                                subscribersb = self.connection_manager.call(
                                    service='sns',
                                    command='list_subscriptions_by_topic',
                                    kwargs=sns_kwargs
                                )
