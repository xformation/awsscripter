...
class Esingest():
    ...
    def get_configuration_snapshot_file(self,s3conn, bucket_name, file_partial_name):
        ...
        result = None
        bucket = self.s3conn.Bucket(str(bucket_name))

        for key in bucket.objects.all():
            if file_partial_name in key.key:
                result = key.key
                self.verbose_log.info("Found the file in S3: " + result)

        return result


    def load_data_into_es(self,filename, iso_now_time, es):
        data = None
            ...
            if len(configuration_items) > 0:
                for item in configuration_items:
                    try:
                        ...
                        self.app_log.error("Couldn't add item: " + str(
                            item) + " because " + str(sys.exc_info()))

        return item_count, couldnotadd_count


    def loop_through_regions(self,cur_region, iso_now_time, es):
        self.app_log.info("Current region: " + cur_region)
        ...
        self.verbose_log.info("Creating snapshot")
        snapshotid = config_service.deliver_snapshot()
        self.verbose_log.info("Got a new snapshot with an id of " + str(snapshotid))
        if snapshotid is None:
            self.app_log.info(
                "AWS Config isn't setup or your requests are being throttled")
            return

        snapshot_file_path = self.get_configuration_snapshot_file(
        ...
        if snapshot_file_path is None:
            self.app_log.error("Snapshot file is empty -- cannot proceed")
            return

        self.app_log.info("Snapshot File Name: " + str(snapshot_file_path))
        ...
        self.verbose_log.info("Loading the file into elasticsearch")
        item_count, could_not_add = self.load_data_into_es(
            DOWNLOADED_SNAPSHOT_FILE_NAME, iso_now_time, es)

        if item_count > 0:
            self.app_log.info("Added: " + str(item_count) +
                         " items into ElasticSearch from " + cur_region)
            exit()
        if could_not_add > 0:
            self.app_log.warn("Couldn't add " + str(could_not_add) +
    ...
    def main(self, es):
        ...
        self.verbose_log.info("Setting up the elasticsearch instance")
        iso_now_time = datetime.datetime.now().isoformat()
        self.app_log.info("Snapshot Time: " + str(iso_now_time))
        ...
        self.verbose_log.info("Looping through the regions")

        for curRegion in my_regions:
            self.loop_through_regions(curRegion, iso_now_time, es)
